...
import '../../shared_ui/basit_markdown.dart';
...
class _DersEkraniState extends State<DersEkrani> {
  ...
  final AkisKontrolcusu _blokAkis = AkisKontrolcusu();
  ...
  Future<void> cevabiKontrolEt([String? gelenCevap]) async {
    if (_dusunmedeMi || _sunumdaMi) return;
    final gorev = _aktifGorev;
    final cevap = gelenCevap ?? _akis.giris.text;

    // Bos cevap, gecis kilidi, ayni cevabin tekrari ve zaten dogru
    // verilmis gorev burada elenir.
    ...
    if (tani.dogru) {
      ...
      // Gecis suresince kilit acik: Enter sonraki gorevi ATLATMAZ.
  ...
  List<Widget> _gorev() {
    ...
    return [
      ...
      const SizedBox(height: 16),
      if (tani != null) _geriBildirim(tani),
      if (!_akis.dogruVerildi) ...[
        const SizedBox(height: 16),
          ...
          child: Column(
            ...
            children: [
              ...
              if (!bitti) ...[
                ...
                CevapAlani(
                  akis: _blokAkis,
                  etiket: 'Bu blogun Ingilizcesi',
                  onGonder: (String v) => _blokCevapla(o.kod, aktif.rolKodu, v),
                ),
                const SizedBox(height: 12),
                FilledButton(
  ...
  /// Blok alistirmasinin merkezi kontrol fonksiyonu.
  /// Her blok cevabi tek kez kaydedilir; ardindan odak alanda kalir.
  void _blokCevapla(String ornekKod, String rolKodu, [String? gelen]) {
    final String v = (gelen ?? _blokAkis.giris.text).trim();
    if (_blokAkis.kilitli || v.isEmpty) return;
    if ((_blokCevaplari[ornekKod] ?? const <String, String>{})
        .containsKey(rolKodu)) {
      return; // ayni blok iki kez kaydedilmez
    }
    setState(() {
      (_blokCevaplari[ornekKod] ??= <String, String>{})[rolKodu] = v;
    });
    // Sonraki blok sorusuna gecildi: alan temizlenir, odak geri gelir.
    _blokAkis.yeniSoru();
  }
